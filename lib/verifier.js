    ...
    for(const file of htmlFiles){
      ...
      if(schema && schema.content_items){
        ...
        try{
          ...
          if(Array.isArray(ci.gallery_or_listings) && ci.gallery_or_listings.length>=6 && counts.gallery < 6) {
            if(file==='index.html') failures.push(`${file}: gallery items ${counts.gallery} <6`);
          }
